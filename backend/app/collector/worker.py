...
async def run_collection_cycle() -> Dict[str, Any]:
    ...
    if _cycle_lock.locked():
        logger.warning(
            "Previous collection cycle is still running. Skipping this interval."
        )
    ...
    async with _cycle_lock:
        ...
        try:
            ...
            if not results:
                ...
                try:
                    firestore.set_collector_status(
                        "warning", start_time, "0 PS records extracted"
                    )
                except Exception:
                    pass
                return {
                ...
                if existing is None:
                    ...
                    if new_count != old_count or title_changed:
                        ...
                        if new_count > old_count:
                            logger.info(
                                f"Count increased for {ps_id}: {old_count} -> {new_count}"
                            )
                            update_data["lastCountChangeAt"] = start_time
                            items_to_update.append((ps_id, update_data))
                            try:
                                firestore.create_history_entry(
                                    ps_id, new_count, old_count, start_time
                                )
                            except Exception as h_err:
                                logger.warning(
                                    f"Failed to create history entry for {ps_id}: {h_err}"
                                )
                            changed_count += 1
                            ...
                            try:
                                if not firestore.notification_event_exists(event_id):
                                    success = send_ps_notification(
                                        ps_id, new_count, capacity
                                    )
                                    if success:
                                        firestore.create_notification_event(
                                            ps_id, new_count, old_count, start_time
                                        )
                                        notification_count += 1
                            except Exception as n_err:
                                logger.warning(
                                    f"Failed processing notification for {ps_id}: {n_err}"
                                )

                        elif new_count < old_count:
                            logger.warning(
                                f"Count decreased for {ps_id}: {old_count} -> {new_count}"
                            )
                            try:
                                firestore.handle_count_decrease(
                                    ps_id, old_count, new_count, start_time
                                )
                            except Exception:
                                pass
                            items_to_update.append((ps_id, update_data))
            ...
            if items_to_set or items_to_update:
                logger.info(
                    f"Writing {len(items_to_set)} new and {len(items_to_update)} updated PS documents to Firestore..."
                )
                try:
                    firestore.batch_write_ps(items_to_set, items_to_update)
                    # Persist baseline history for newly discovered PSs so the graph has saved data
                    # from the very first observation (last update time == firstSeenAt initially).
                    for ps_data in items_to_set:
                        try:
                            ...
                            logger.warning(
                                f"Failed to create initial history for {ps_data['ps_id']}: {h_err}"
                            )
                except Exception as b_err:
                    logger.error(f"Firestore batch write error: {b_err}")
            else:
                logger.info(
                    "No count changes detected across 233 PSs — 0 Firestore writes needed (Quota saved) ✓"
                )
            ...
            logger.info(
                f"Collection cycle completed successfully in {duration_ms}ms ({len(results)} PSs checked, {changed_count} changed)"
            )
            ...
            result_summary.update(
                {"status": "failed", "durationMs": duration_ms, "error": str(err)}
            )
